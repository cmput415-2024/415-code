...
std::shared_ptr<Symbol> MethodSymbol::resolve(const std::string &name) {
    ...
    std::shared_ptr<Scope> enclosingScope = getEnclosingScope();
    std::shared_ptr<ClassSymbol> classScope = std::dynamic_pointer_cast<ClassSymbol>(enclosingScope);
    if (classScope) {
        std::shared_ptr<Symbol> sym = classScope->resolveMember(name);
        if (sym) return sym;
    }
    if (enclosingScope) {
